...
function connectSocket() {
  ...
  socket.on('icons_refreshed',d=>showToast(d.success?'Icônes actualisées':'Erreur icônes',d.success?'success':'error'));
...
function renderQuickActions(){const bar=$('bottomActions'),acts=configData?.quick_actions||[];bar.innerHTML=acts.map(a=>`<button class="qa-btn" data-id="${a.id}">${a.icon_b64?'<img src="'+a.icon_b64+'" style="width:18px;height:18px">':'<span class="material-symbols-rounded">'+(a.icon||'flash_on')+'</span>'}${a.name}</button>`).join('');bar.querySelectorAll('.qa-btn').forEach(b=>b.addEventListener('click',()=>{const a=acts.find(x=>x.id===b.dataset.id);if(!a)return;if(a.type==='screenshot'){socket.emit('screenshot');showToast('Capture...','info');}else if(a.confirm)showModal(a.name,`Exécuter "${a.name}" ?`,()=>launchApp(a.id));else launchApp(a.id);}));}
...
$('refreshIconsBtn').addEventListener('click',()=>{if(!connected)return showToast('Non connecté','error');showToast('Actualisation des icônes...','info');socket.emit('refresh_icons');});
